...
we send quotes on the event bus. Traders are components that consume these two services with only one goal: getting
rich (or not...)!

In this section, we are going to develop 3 traders (following a stupid logic you can definitely improve):

* the first trader is developer with RX Java 2
* the second trader is developed with Kotlin
* the third trader is developed in Scala
